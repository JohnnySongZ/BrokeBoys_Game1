...
public static class Networking
{
    ...
    {
        public Action<SocketState> action;
        public TcpListener listener;

        public ConnectionInfo(Action<SocketState> action, TcpListener listener)
        {
            this.action = action;
    ...
    public static TcpListener StartServer(Action<SocketState> toCall, int port)
    {
        ...
        {
            listener.Start();
            ConnectionInfo ci = new(toCall, listener);
            listener.BeginAcceptSocket(AcceptNewClient, ci);
        }
        catch (Exception e)
        {
    ...
    {
        ConnectionInfo connect = (ConnectionInfo)ar.AsyncState!;
        SocketState ss;
        Socket socket;
        try
        {
            socket = connect.listener.EndAcceptSocket(ar);
            ss = new SocketState(connect.action, socket);
            ss.OnNetworkAction(ss);
            connect.listener.BeginAcceptSocket(AcceptNewClient, connect);
        }
        catch (Exception e)
        {
            SocketState exState = new SocketState(connect.action, "Exception while accepting new client");
            exState.OnNetworkAction(exState);
        }
    }
    ...
    public static void ConnectToServer(Action<SocketState> toCall, string hostName, int port)
    {
        ...
        try
        {
            ...
            if (!foundIPV4)
            {
                ...
                SocketState ss = new(toCall, s);
                ss.ErrorOccurred = true;
                ss.ErrorMessage = "Error Occured";
                toCall(ss);
            }
        }
        catch (Exception)
        ...
        try
        {
            IAsyncResult asyncResult = socket.BeginConnect(ipAddress, port, ConnectedCallback, new SocketState(toCall, socket));
            bool worked = asyncResult.AsyncWaitHandle.WaitOne(3000, true);
            if (!worked)
            {
                socket.Close();
            }
        ...
        {
            SocketState ss = new(toCall, socket);
            ss.ErrorOccurred = true;
            ss.ErrorMessage = "Error Occured";
            toCall(ss);
        }
    ...
    private static void ConnectedCallback(IAsyncResult ar)
    {
        ...
        {
            SocketState exState = new SocketState(ss.OnNetworkAction, "Error in connectedcallback");
            exState.OnNetworkAction(exState);
        }
    }
    ...
    public static void GetData(SocketState state)
    {
        ...
        catch (Exception)
        {
            ...
            state.ErrorMessage = "Error occurred in GetData";
    ...
    private static void ReceiveCallback(IAsyncResult ar)
    {
        ...
        try
        {
            int num = ss.TheSocket.EndReceive(ar);
            if (num > 0)
            {
                lock (ss.data)
                {
                    ss.data.Append(Encoding.UTF8.GetString(ss.buffer, 0, num));
                }
            }
            ss.OnNetworkAction(ss);
        }
        ...
        {
            ss.ErrorOccurred = true;
            ss.ErrorMessage = "Error occured in ReceiveCallback";
            ss.OnNetworkAction(ss);
        }
    }
