...
public class NetworkTests
{
    ...
    /// <summary>
    /// Test send and close. Calls send after send and close which does not send since the
    /// socket is closed by send and close
    /// </summary>
    /// <param name="clientSide"></param>
    [DataRow(true)]
    [DataRow(false)]
    ...
    /// <summary>
    /// Tests that SendAndClose will return false since the socket is closed by an earlier SendAndClose call
    /// </summary>
    /// <param name="clientSide"></param>
    [DataRow(true)]
    [DataRow(false)]
    [DataTestMethod]
    public void TestDelayedSendAndCloseSocketAlreadyClosed(bool clientSide)
    {
        SetupTestConnections(clientSide, out testListener, out testLocalSocketState, out testRemoteSocketState);
    ...
    /// <summary>
    /// Tests that send returns false when data is null
    /// </summary>
    /// <param name="clientSide"></param>
    [DataRow(true)]
    [DataRow(false)]
    [DataTestMethod]
    public void TestSendNullData(bool clientSide)
    {
        SetupTestConnections(clientSide, out testListener, out testLocalSocketState, out testRemoteSocketState);
        ...
        Assert.IsFalse(Networking.Send(testLocalSocketState.TheSocket, null));
    }
}